...
const WorkingDesk = () => {
    const cameraTarget: [number, number, number] = [0, 2, 1.25];
    const tableRef = useRef(null);
    const { position } = useControls('table', {
        position: {
            x: -0.3,
            y: 0,
            z: -3
    ...
    const { chairPosition } = useControls('chair', {
        chairPosition: {
            x: -0.1,
            y: 0,
            z: 1.25
    ...
    return (
        <>
            ...
            {/* <group ref={tableRef} scale={0.5} rotation={[0, Math.PI, 0]} position={[position.x, position.y, position.z]}>
                <ComputerDesk />
                <Html transform wrapperClass="html-screen" distanceFactor={0.55} position={[1, 0, -5]} rotateX={-0.256}>
