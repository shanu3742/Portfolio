...
const PalmTreeInstance = ({ scaleIndex = 1 }) => {
  ...
  const { nodes: palmNodes, materials: palmMaterial } = useGLTF('/model/tree/PalmTree.gltf') as any;
  const { nodes: pineNodes, materials: pineMaterial } = useGLTF('/model/tree/PineTree.gltf') as any;
  ...
  return (
    <>
      ...
      >
        {trees.map((tree, i) => {
          return <Instance key={i} position={(tree.position) as any} scale={(scale) as any} />
        })}
      </Instances>
    </>
  );
